...
__device__ __forceinline__ void read640(uint32_t* data, void* pagesPtr, void* pageBase, uint32_t offset) {
  ...
  if(offset+640<=PAGE_SIZE-4) {
    data[0]=*(uint32_t*)byteOffset(pageBase, offset+warpThread*4+0);//32 thread read 128 bytes=rank size
...
template<class Offsets>
__device__ __forceinline__ void prefixSumBuckets(uint32_t block, uint32_t base) { //2^12 scope
  ...
  for(int32_t i=0;i<8;i++) {
    if(i<stop) {//pointCounts->4096*4
      ...
      if(halfWarpThread==15)
        store_shared_u32(Offsets::bufferCountsOffset + i*halfWarps*4 + halfWarp*4, sum);//4096/16=256
  ...
  if(threadIdx.x<256) {//针对这256个数字，再次计算前缀和
    count=load_shared_u32(Offsets::bufferCountsOffset + threadIdx.x*4);
    sum=multiwarpPrefixSum(Offsets::warpSumsOffset, count, 8);//在前8个warps内计算总和
    store_shared_u32(Offsets::prefixCountsOffset + threadIdx.x*4, sum - count);//pre sum of  256 counts
  }
...
__device__ __forceinline__ void sortMap(uint32_t block, uint32_t scratchCount) {
  ...
  if(threadIdx.x<256) {//use 1/4 threads?
    count=load_shared_u32(Offsets::bufferCountsOffset + threadIdx.x*4);
  ...
  for(uint32_t scratchIndex=threadIdx.x;scratchIndex<scratchCount;scratchIndex+=blockDim.x) {
    ...
    mapEntry=(bin<<24) + scratchIndex;//subwin position && scratch:lowBits+point-index
...
__device__ __forceinline__ void writePointToShared(uint32_t block, void* scratchPtr, uint32_t lowBits, uint32_t highBits, bool valid) {
  ...
  while(!__all_sync(0xFFFFFFFF, processed)) {
    offset=0;
    if(!processed) {
      bin=__funnelshift_l(lowBits, highBits, 1);//bin ={0-255}
      ...
      if(offset<=124) {
        ...
        // store 256*128
        store_shared_u32(Offsets::buffersOffset + bin*128 + offset, lowBits & 0x7FFFFFFF);// 31bit = 4bit lowBits + sign bit + 26 bits point-index
        processed=true;
      }
    }
    ...
    while(true) {// in every warp
      ...
      *(uint32_t*)byteOffset(scratchPtr, shuffleIndex*128 + warpThread*4)=data;//一次写入128*256长度的sign-bit && point-index数据
...
__device__ __forceinline__ void partitionPagesToScratch(uint32_t block, void* scratchPtr, void* pagesPtr, uint32_t size) {
  ...
  offset=warp*640;
  idx=warp*128;
  while(idx<size) {//size 平均数2^16/11,
    while(offset>=PAGE_SIZE-4) { //一个size可能分给多个page
      nextPage=*(uint32_t*)byteOffset(currentPage, PAGE_SIZE-4);
      currentPage=byteOffset(pagesPtr, nextPage, PAGE_SIZE);
      offset-=PAGE_SIZE-4;
    }
    if(idx<size)//warp 组内，一次读取640字节数据
      read640(data, pagesPtr, currentPage, offset);
    unpackData(lowBits, highBits, data);//scalar && point-index  //一个size对应的1个sclar
    #pragma unroll
    for(int32_t i=0;i<4;i++) {
    ...
    offset+=warps*640;//每次处理20480 length数据
    idx+=warps*128;//+=4kb
  }
...
__device__ __forceinline__ void partitionScratchToPoints(uint32_t block, void* pointsPtr, void* scratchPtr, uint32_t scratchCount, uint32_t points) {
  ...
  for(uint32_t scratchIndex=warp;scratchIndex<scratchCount;scratchIndex+=warps) {
    mapEntry=load_shared_u32(Offsets::sortedMapOffset + scratchIndex*4);//mapEntry store bin value, which is 2^8 bits of sub-scalar
    point=*(uint32_t*)byteOffset(scratchPtr, (mapEntry & 0x00FFFFFF)*128 + warpThread*4);
    bucket=(mapEntry>>20) + (point>>27);//bucket:0~4096
...
__global__ void partition4096Kernel(void* pointsPtr, void* unsortedTriplePtr, void* scratchPtr, void* prefixSumSizesPtr, 
  ...
  while(true) {
    if(threadIdx.x==0) {
      ...
      printf("block:%d\n", block);// A40, max block 84??
      store_shared_u32(Offsets::countersOffset + 4, block);
    }

    __syncthreads();//sync in block
    ...
    if(block>=11*1024)
      break;
    //block is all subWindows's index
    initializeShared<Offsets>(block);

    size=*(uint32_t*)byteOffset(sizesPtr, block*4);//每个子窗口（2^12）中含有的点数总和??
    prefixSumSize=*(uint32_t*)byteOffset(prefixSumSizesPtr, block*4);//子窗口（2^12）前面的所有子窗口中点的总数??
     
    __syncthreads();
  
    if(size<SIZE_LIMIT)//相同globalBin的数据量不会超过11*1024个page
      partitionPagesToScratch<Offsets>(block, scratchPtr, pagesPtr, size);//scalar
    else
    ...
    if(size<SIZE_LIMIT)
      partitionScratchToPoints<Offsets>(block, pointsPtr, scratchPtr, scratchCount, points); //point
    else
