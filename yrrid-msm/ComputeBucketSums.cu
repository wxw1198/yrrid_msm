...
__device__ __forceinline__ uint32_t copyCountsAndIndexes(uint32_t countsAndIndexesOffset, uint4* sortedCountsAndIndexes, uint32_t bucket) {
  ...
  // copy 1536 bytes =>384个并发线程，每个线程读取6个字节
  ...
  count+=load.x + load.z;
  load.y=load.y<<2;//count个数转成字节距离
  load.w=load.w<<2;//count个数转成字节距离
...
__device__ __forceinline__ void copyPointIndexes(uint32_t& sequence, uint32_t countsAndIndexesOffset, uint32_t pointIndexOffset, void* pointIndexes, uint32_t bucket) {
  ...
  while(remaining>0) {
    ...
    if(countAndIndex.x==0) {
      ...
      quad=*(uint4*)byteOffset(pointIndexes, countAndIndex.y);//y is 4096个全局（11*windows）前缀和; 一次读取4个point-indexs(16 bytes)

      shift=shift>>2;
      //对齐读取pointindexs数据的边界countAndIndex.y从4*4bytes开始
      available=umin(countAndIndex.x, 4-shift);//x =>count, y==>index
    ...
    countAndIndex.x-=available;//available max value 4?
  ...
  countAndIndex.y-=available<<2;
  if(countAndIndex.x>0) {//count未被处理完毕
    sequence-=256;
...
__device__ __forceinline__ void prefetch(uint32_t storeOffset, uint32_t pointIndex, void* pointsPtr) {
  ...
  //read 1 point,96bytes
  shared_async_copy_u4(storeOffset+0, byteOffset(p0, 0 + oddEven*16));//in warp, 32threads,once read 128bytes
  shared_async_copy_u4(storeOffset+32, byteOffset(p0, 32 + oddEven*16)); //u4=4*u32=16 bytes
...
__global__ void computeBucketSums(void* bucketsPtr, void* pointsPtr, void* sortedTriplePtr, void* pointIndexesPtr, void* atomicsPtr) {
  ...
  //    xxxx - yyyyy    36864         point data  //store 1152 points
  ...
  while(true) {
    ...
    while(__any_sync(0xFFFFFFFF, count>0)) {
      ...
      #pragma unroll 1
      for(int i=1;__any_sync(0xFFFFFFFF, count>0) && i<=GROUP;i++) {//一次缓存数据的量是一个GROUP
        prefetchWait();
        __syncwarp(0xFFFFFFFF);
