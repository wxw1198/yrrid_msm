...
__global__ void processSignedDigitsKernel(void* processedScalarData, void* scalarData, uint32_t points) {
  ...
  if(threadIdx.x==1) distributed=0x0a118000;//168919040
  if(threadIdx.x==2) distributed=0xd0000001;//3489660929
  ...
  for(int32_t i=1;i<15;i++) {
    current=uadd_cc(current, distributed);//current 作用??
    carry=uaddc(0, 0)!=0;
  ...
  for(int32_t i=0;i<11;i++) {
    ...
    sliced[i]=compress(sliced[i]);//warp内的线程同步，实现数据压缩
  }

  if(warpThread<24) {//大于24的warpThread不需要处理数据，数据已经被小于24的线程处理完毕
    #pragma unroll
    for(int32_t i=0;i<11;i++) {
      base=byteOffset(processedScalarData, i, 3*points);//点切分后，有maxPoints*33个subScalars
