...
__device__ __forceinline__ void cleanup(void* pagesPtr, void* sizesPtr, void* countersPtr, uint32_t window, uint32_t localBin, uint32_t writeBytes) {
  ...
  while(true) {
    mask=__ballot_sync(0xFFFFFFFF, currentWriteBytes>0);//线程 predicate 为零的该函数返回值该位为 0，线程 predicate 非零的该函数返回值该位为 1 。
    if(mask==0)
      return;
...
__device__ __forceinline__ void processWrites(void* pagesPtr, void* sizesPtr, void* countersPtr, bool writeRequired, uint32_t globalBin, uint32_t highBitMask) {
  ...
  if(writeRequired) {
    while(true) {
      page=atomicAdd((uint64_t*)byteOffset(countersPtr, globalBin*8 + 8), 60ull);//一次写入60个字节
      if(ulow(page)==PAGE_SIZE-64) {//mem full
        newPage=nextPage(countersPtr);
        atomicExch((uint64_t*)byteOffset(countersPtr, globalBin*8 + 8), make_wide(0, newPage));//record max page
        atomicAdd((uint32_t*)byteOffset(sizesPtr, globalBin*4), 1);//at postion globalBin*4, page total number
        break;
      }
  ...
  while(true) {
    ...
    if(mask==0)
      return;
    thread=31-__clz(mask);//thread 是在warp内，每个线程的warpThread
    //广播thread的值给其它同warp中的线程
    //thread 相对于函数中的掩码偏移,example: mask:0xffffffff=32threads,thread=2,表示第29warpThread 的 writeRequired=true
    ...
    if(warpThread<15) {
      ...
      data=shared_atomic_exch_u32(shuffleMemoryOffset + warpThread*4, 0xFFFFFFFF);//从共享内存中读取scalars
    }
    ...
    if(warpThread<writeThreads)//数据写入到共享缓存与从共享缓存中读取，写入到全局共享缓存，使用的线程是不同的
      *(uint32_t*)byteOffset(pageBase, shufflePageLow + warpThread*4)=data; //in data ,is scalar
  }
}
...
__global__ void partition1024Kernel(void* pagesPtr, void* sizesPtr, void* countersPtr, void* processedScalarsPtr, uint32_t points) {
  ...
  uint32_t writeLowBytes, writeHighByte, bin, sharedBase, offset, chunkSize=16384;//2^14
  bool     processed;
  void*    chunkBase;
  ...
  while(true) {
    if(window*points+points<=chunk) {
      ...
      while(window<11 && window*points+points<=chunk) {
        window++;
        cooperative_groups::this_grid().sync(); //sync in grid
      }
    ...
    for(int32_t i=threadIdx.x;i<chunkSize;i+=blockDim.x) {//loop 16 times
      point=(chunk-window*points)+i;//point index
      if(warpThread<24)
        scalar=*(uint32_t*)byteOffset(chunkBase, (i>>5)*96 + warpThread*4);//与存入processedScalars中一致
      ...
      while(!__all_sync(0xFFFFFFFF, processed)) {//当且仅当所有线程的 predicate 非零时返回 1，否则返回 0。
        offset=0;
        if(!processed) {
